...
router.get('/v1/ssoAuth/login', passport.authenticate('saml', config.saml.options), (req, res, next) => {
    return res.redirect('http://localhost:3000');
});

router.post('/v1/ssoAuth/loginCallback', passport.authenticate('saml', config.saml.options), (req, res, next) => {
    return res.redirect('http://localhost:3000');
});
